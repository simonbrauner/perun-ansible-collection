...
def get_mails(params, api_client):
    ...
    if params["vo_id"] is not None:
        response = manager.get_application_mails_for_vo(
            int(params["vo_id"]), _preload_content=False
        )
    else:
        response = manager.get_application_mails_for_group(
            int(params["group_id"]), _preload_content=False
        )
